...
const TrackRow = ({ track }) => (
    <a
        ...
        className="group flex items-center gap-3 rounded-xl bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 p-2.5 transition-colors duration-200"
    >
        {track.image ? (
            <img src={track.image} alt="" className="w-11 h-11 rounded-lg object-cover shrink-0" />
        ) : (
            <div className="w-11 h-11 rounded-lg bg-black/10 dark:bg-white/10 shrink-0"></div>
        )}
...
const SkeletonRow = () => (
    <div className="flex items-center gap-3 rounded-xl bg-black/5 dark:bg-white/5 p-2.5 animate-pulse">
        <div className="w-11 h-11 rounded-lg bg-black/10 dark:bg-white/10 shrink-0"></div>
        <div className="flex-1 space-y-2">
            <div className="h-3 w-2/3 rounded bg-black/10 dark:bg-white/10"></div>
            <div className="h-2.5 w-1/3 rounded bg-black/10 dark:bg-white/10"></div>
        </div>
    </div>
);
...
const Spotify = () => {
    ...
    return (
        <div className="bg-white dark:bg-[#171717] rounded-2xl p-4 sm:p-5 shadow-lg ring-1 ring-black/10 dark:ring-white/10 text-black dark:text-white mx-auto w-full transition-colors duration-500">
            {/* Header */}
            <div className="flex items-center justify-between gap-3 mb-4">
                <h2 className="text-sm sm:text-base font-semibold">
                    {tabs.find((t) => t.key === activeTab)?.label}
                </h2>
                <div className="flex rounded-lg bg-black/5 dark:bg-white/5 p-1 transition-colors duration-500">
                    {tabs.map((tab) => (
                        <button
                            key={tab.key}
            ...
            {loading ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="rounded-xl bg-black/5 dark:bg-white/5 aspect-square animate-pulse"></div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    {featured && (
                        <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="group relative flex flex-col justify-end rounded-xl overflow-hidden bg-black/5 dark:bg-white/5 min-h-56 sm:min-h-0"
                        >
                            {featured.image && (
                                <img
                                    src={featured.image}
